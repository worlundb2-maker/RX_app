...
const roleOrder = ['pharmacist', 'technician', 'clerk', 'driver', 'misc'];
export const STAFFING_PROFILE = [
    {
...
function operatingDayWeight(dateValue) {
    if (!dateValue)
        return 0;
...
function capacityStatus(available, stretchNeed, preferredNeed) {
    if (preferredNeed === 0)
        return 'No demand observed';
    if (available >= preferredNeed)
        return 'At or above preferred coverage';
    if (available >= stretchNeed)
        return 'Within stretch range';
    if (available > 0)
        return 'Below minimum coverage';
    return 'Open coverage needed';
}
function pressureLevel(status) {
    if (/below minimum|open coverage/i.test(status))
        return 'high';
    if (/stretch/i.test(status))
        return 'medium';
    return 'low';
}
function staffingNeed(avgRxPerWeightedDay, role) {
    if (role === 'pharmacist') {
...
export function buildStaffingState(pioneerClaims) {
    const perPharmacy = STAFFING_PROFILE.map((profile) => {
        ...
        const operatingDates = [...new Set(claims.map((claim) => claim.fillDate || claim.claimDate).filter(Boolean))];
        ...
        return {
            ...
            roles: roleRows,
            roster: profile.roles.reduce((acc, role) => {
