...
import { addUser, clearDataset, ingestInboxDir, PHARMACIES, pharmacyByCode, readDb, saveUpload, setReviewDecision } from './data.js';
import { ingestUpload } from './parser.js';
import { getAppState } from './analysis.js';
...
function resolveInboxPharmacy(scope) {
    const cleaned = String(scope || '').trim().toUpperCase();
    if (!cleaned)
        return undefined;
    const normalized = cleaned.replace(/[^A-Z0-9]+/g, '_');
    if (normalized === 'MV' || normalized === 'MONTEVISTA' || normalized === 'MONTE_VISTA')
        return 'MONTE_VISTA';
    if (normalized === 'ARLINGTON')
        return 'ARLINGTON';
    if (normalized === 'KONAWA')
        return 'KONAWA';
    if (normalized === 'SEMINOLE')
        return 'SEMINOLE';
    return PHARMACIES.find((pharmacy) => pharmacy.code === normalized)?.code;
}
function detectInboxTypeFromTokens(tokens) {
    ...
    if (has('pioneer') && has('claims'))
        return 'pioneer';
    if (has('inventory') || has('onhand', 'onhands'))
        return 'inventory';
    if (has('mtf')) {
        if (has('adjustment', 'adjustments'))
            return 'mtf_adjustment';
        if (has('payment', 'payments'))
            return 'mtf';
        return 'mtf';
    }
    return null;
}
function parseInboxAssignment(fileName) {
...
function scanInbox() {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            saveUpload({ originalname: originalName, filename: storedName }, assignment.type, assignment.pharmacyCode || 'ALL', result.impactedPharmacies, result.rows, result.sourceRows, result.rejectedRows, result.sheetName);
            imported.push({
                file: originalName,
...
export function registerRoutes(app) {
    ...
    app.get('/api/state', (req, res) => {
        const pharmacyCode = typeof req.query.pharmacyCode === 'string' && req.query.pharmacyCode !== 'ALL' ? req.query.pharmacyCode : undefined;
    ...
    app.post('/api/upload', upload.single('file'), (req, res) => {
        try {
            ...
            const pharmacyCode = (req.body.pharmacyCode || undefined);
            if (!req.file || !type)
                return res.status(400).json({ message: 'file and type are required' });
            if (type === 'inventory' && !pharmacyByCode(pharmacyCode))
                return res.status(400).json({ message: 'Inventory uploads require a pharmacy selection' });
    ...
    app.post('/api/clear', express.json(), (req, res) => {
        clearDataset((req.body?.dataset || 'all'));
        res.json({ ok: true });
    });
}
