...
function asHeaderArray(row: any[]): string[] {
...
function scanWorkbook(workbook: xlsx.WorkBook, type: UploadType): SheetSpec | null {
  const candidates: SheetSpec[] = [];
  ...
  const ranked = candidates.sort((a, b) => {
    ...
    if (b.sourceRows !== a.sourceRows) return b.sourceRows - a.sourceRows;
...
function pickBestSheet(workbooks: xlsx.WorkBook[], type: UploadType): SheetSpec {
  const specs = workbooks.map((workbook) => scanWorkbook(workbook, type)).filter(Boolean) as SheetSpec[];
...
function findValue(row: RowObject, candidates: string[]) {
...
function findValueStrict(row: RowObject, candidates: string[]) {
  const map = valueMap(row);
  for (const candidate of candidates) {
    const value = map.get(normalizeKey(candidate));
    if (value !== undefined && value !== null && String(value).trim() !== '') return value;
...
function cleanNdc(value: any): string {
...
function mergeByKey<T>(existing: T[], incoming: T[], keyFn: (row: T) => string) {
  const incomingKeys = new Set(incoming.map(keyFn));
  return [...existing.filter((row) => !incomingKeys.has(keyFn(row))), ...incoming];
}

function inventoryGroup(value: any): InventoryGroup {
...
function rowsFromFile(filePath: string, type: UploadType): { rows: RowObject[]; meta: ParseMeta } {
...
function amountFromMtf(row: RowObject, type: UploadType): { normalized: number | null; raw: number | null; unexpected: boolean; reason: string | null } {
  ...
  if (raw == null) return { normalized: null, raw: null, unexpected: false, reason: null };
...
function parsePriceRow(row: RowObject, group: InventoryGroup): PriceRow | null {
...
function finalizeResult(rows: number, meta: ParseMeta, impactedPharmacies: PharmacyCode[]): UploadIngestResult {
...
export function ingestUpload(filePath: string, type: UploadType, requestedPharmacy?: PharmacyCode): UploadIngestResult {
  ...
  if (type === 'pioneer') {
    const claims = rows.map((row) => {
      ...
      } satisfies PioneerClaim;
    }).filter(Boolean) as PioneerClaim[];
  ...
  if (type === 'mtf' || type === 'mtf_adjustment') {
    const claims = rows.map((row) => {
      ...
      if (!rxNumber && !icn) return null;
      if (payment.raw == null) return null;
      if (type === 'mtf_adjustment' && payment.raw >= 0) return null;
      return {
        id: randomUUID(),
        pharmacyCode: pharmacy.code,
        pharmacyNpi: pharmacy.npi,
  ...
  if (type === 'inventory') {
    ...
    if (!pharmacy) throw new Error('Inventory uploads require a pharmacy selection');
  ...
  if (type === 'price_rx' || type === 'price_340b') {
    ...
    const priceRows = rows.map((row) => parsePriceRow(row, group)).filter(Boolean) as PriceRow[];
