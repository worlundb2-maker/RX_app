import { PHARMACIES, readDb } from './data.js';
import { buildStaffingState } from './staffing.js';
...
const sdraMap = new Map(SDRA_REFERENCE);
...
function cleanNdc(ndc) {
    return String(ndc || '').replace(/[^0-9]/g, '');
}
function isInactiveLifecycle(claim) {
    const lifecycle = String(claim.normalizedClaimLifecycle || '').toLowerCase();
    if (['reversed', 'cancelled', 'rejected_on_hold', 'transferred', 'other_inactive'].includes(lifecycle))
        return true;
    ...
    if (claimType === 'B2')
        return true;
    if (/cancel|reverse|reject|hold|transfer/.test(currentStatus))
        return true;
    return false;
}
function activeClaimsOnly(claims) {
    return claims.filter((claim) => !isInactiveLifecycle(claim));
}
function countLifecycle(claims, lifecycle) {
...
function reviewLabelDisplay(label) {
    if (label === 'flag')
        return 'Flag';
    if (label === 'do_not_flag')
        return 'Do not flag';
    if (label === 'resolved')
        return 'Resolved';
    return '';
}
function applyReviewDecisions(rows, reviewDecisionMap) {
    return rows.map((row) => {
        const decision = reviewDecisionMap.get(row.id);
        if (!decision)
            return { ...row, manualLabel: '' };
...
function buildStockSizeMap(rows) {
    const exact = new Map();
    const anyGroup = new Map();
    for (const row of rows) {
        if (row.stockSize == null || row.stockSize <= 0)
            continue;
        exact.set(`${row.pharmacyCode}|${cleanNdc(row.ndc)}`, Number(row.stockSize));
        if (!anyGroup.has(`${row.pharmacyCode}|${cleanNdc(row.ndc)}`))
            anyGroup.set(`${row.pharmacyCode}|${cleanNdc(row.ndc)}`, Number(row.stockSize));
    }
    return { exact, anyGroup };
}
function isOneStockSizeCycle(claim, stockSizeMap) {
    const daysSupply = Number(claim.daysSupply || 0);
    if (![7, 14, 28].includes(daysSupply))
        return false;
...
function buildInventoryReferenceMap(rows) {
    const map = new Map();
    for (const row of rows) {
        const key = `${row.pharmacyCode}|${cleanNdc(row.ndc)}`;
        if (!map.has(key))
            map.set(key, { drugName: row.drugName, strength: row.strength, dispensingUnit: row.dispensingUnit });
    }
    return map;
}
function daySupplyReferenceText(claim, inventoryReferenceMap) {
...
function isPackageSensitiveDaySupplyClaim(claim, inventoryReferenceMap) {
    const text = daySupplyReferenceText(claim, inventoryReferenceMap);
    const form = dosageFormKey(text);
    if (form === 'injectable' || form === 'inhaler' || form === 'neb' || form === 'drops')
        return true;
...
function isVariableDosePackageClaim(claim, stockSizeMap, inventoryReferenceMap) {
    const quantity = Number(claim.quantity || 0);
    const daysSupply = Number(claim.daysSupply || 0);
    if (!quantity || !daysSupply)
        return false;
    const stockSize = stockSizeMap.exact.get(`${claim.pharmacyCode}|${cleanNdc(claim.ndc)}`)
        ?? stockSizeMap.anyGroup.get(`${claim.pharmacyCode}|${cleanNdc(claim.ndc)}`)
        ?? 0;
    if (stockSize <= 0 || Math.abs(quantity - stockSize) > 0.0001)
        return false;
    return isPackageSensitiveDaySupplyClaim(claim, inventoryReferenceMap);
}
function hasAtypicalDaySupply(claim, stockSizeMap, inventoryReferenceMap) {
    const quantity = Number(claim.quantity || 0);
    const daysSupply = Number(claim.daysSupply || 0);
    if (!quantity || !daysSupply)
        return false;
    if (isOneStockSizeCycle(claim, stockSizeMap))
        return false;
    if (isVariableDosePackageClaim(claim, stockSizeMap, inventoryReferenceMap))
        return false;
    const unitsPerDay = quantity / daysSupply;
    if (isPackageSensitiveDaySupplyClaim(claim, inventoryReferenceMap) && unitsPerDay < 0.35)
        return false;
...
function claimFamilyKey(claim, priceMaps) {
    ...
    if (reference)
        return priceFamilyKey(reference);
...
function payerBucket(name) {
    const payer = name.toLowerCase();
    if (payer.includes('caremark') || payer.includes('cvs'))
        return 'Caremark';
    if (payer.includes('express') || payer.includes('esi'))
        return 'Express Scripts';
    if (payer.includes('optum'))
        return 'Optum';
    if (payer.includes('humana'))
        return includesMedicaidMarker(name) ? 'Medicaid' : 'Humana';
    if (payer.includes('prime'))
        return 'Prime';
    if (includesMedicaidMarker(name))
        return 'Medicaid';
    if (payer.includes('cash') || payer.includes('self-pay') || payer.includes('rx cash'))
        return 'Cash';
    return name || 'Unknown';
}
...
function rxFillKey(pharmacyCode, rxNumber, fillNumber) {
    return `${pharmacyCode}|${rxNumber}|${fillNumber}`;
}
function rxOnlyKey(pharmacyCode, rxNumber) {
    return `${pharmacyCode}|${rxNumber}`;
}
function lookupExpectedSdra(claim) {
...
function totalRemitForClaim(claim) {
    ...
    if (claim.primaryRemitAmount != null || claim.secondaryRemitAmount != null)
        return 0;
    if (claim.totalPricePaid != null)
        return money(Number(claim.totalPricePaid || 0));
    return null;
}
function acquisitionForClaim(claim, priceMaps) {
    if (claim.acquisitionCost != null)
        return money(Number(claim.acquisitionCost || 0));
...
function payerDisplayName(claim) {
    if (claim.payerType === 'Cash')
        return claim.thirdPartyName || claim.primaryPayer || 'RX CASH';
    return claim.thirdPartyName || claim.primaryPayer || 'Unknown';
}
function pharmacyNameOf(code) {
    return PHARMACIES.find((row) => row.code === code)?.name || code;
}
function unexpectedPaymentAmount(row) {
...
function recentWindowCutoff(claims) {
    const dates = claims.map((claim) => claim.fillDate || claim.claimDate).filter(Boolean);
    if (!dates.length)
        return null;
...
function positiveShortfall(value) {
    return value > 0 ? value : 0;
}
function grossProfitTuple(claim, priceMaps) {
    const remit = totalRemitForClaim(claim);
    const acquisition = acquisitionForClaim(claim, priceMaps);
    if (remit == null || acquisition == null)
        return null;
...
function classifySdraStatus(args) {
    ...
    if (claim.inventoryGroup === '340B') {
        if (netPayment > 1)
            return '340B paid and should not have been';
        if (netPayment < -1)
            return '340B adjustment posted';
        return '340B no payment expected';
    }
    if (Math.abs(netPayment - expected) <= 1)
        return 'RX paid correctly';
    if (netPayment > 1)
        return 'RX not paid correctly';
    if (fillDate && pendingCutoff && fillDate >= pendingCutoff)
        return 'Pending payment';
    return 'RX not paid and should have been';
}
function buildPriceMaps(priceRows) {
    const byGroupExact = new Map();
    const byAnyExact = new Map();
    const byGroupFamily = new Map();
    const weightedFamily = new Map();
    const rowScore = (row) => Number(Boolean(normalizeGcn(row.gcn)))
    ...
    for (const row of priceRows) {
        ...
        if (!existing || rowScore(row) > rowScore(existing))
            byAnyExact.set(ndc, row);
        const familyKey = priceFamilyKey(row);
        const familyBucket = `${row.inventoryGroup}|${familyKey}`;
        if (!byGroupFamily.has(familyBucket))
            byGroupFamily.set(familyBucket, []);
        byGroupFamily.get(familyBucket).push(row);
        if (!weightedFamily.has(familyKey))
            weightedFamily.set(familyKey, []);
        weightedFamily.get(familyKey).push(row);
    }
    for (const rows of byGroupFamily.values())
        rows.sort((a, b) => Number(a.acquisitionCost || Infinity) - Number(b.acquisitionCost || Infinity));
    for (const rows of weightedFamily.values())
        rows.sort((a, b) => Number(a.acquisitionCost || Infinity) - Number(b.acquisitionCost || Infinity));
    return { byGroupExact, byAnyExact, byGroupFamily, weightedFamily };
}
export function getAppState(pharmacyCode) {
    const db = readDb();
    ...
    const mtfByExact = new Map();
    const mtfByRxFill = new Map();
    const mtfByRxOnly = new Map();
    for (const row of mtfClaims) {
        if (!mtfByExact.has(paymentKey(row)))
            mtfByExact.set(paymentKey(row), []);
        mtfByExact.get(paymentKey(row)).push(row);
        if (!mtfByRxFill.has(rxFillKey(row.pharmacyCode, row.rxNumber, row.fillNumber)))
            mtfByRxFill.set(rxFillKey(row.pharmacyCode, row.rxNumber, row.fillNumber), []);
        mtfByRxFill.get(rxFillKey(row.pharmacyCode, row.rxNumber, row.fillNumber)).push(row);
        if (!mtfByRxOnly.has(rxOnlyKey(row.pharmacyCode, row.rxNumber)))
            mtfByRxOnly.set(rxOnlyKey(row.pharmacyCode, row.rxNumber), []);
        mtfByRxOnly.get(rxOnlyKey(row.pharmacyCode, row.rxNumber)).push(row);
    }
    const usedMtfIds = new Set();
    ...
    const claimsAnalysisRaw = claimGroups.map((claims) => {
        ...
        if (materialNegativeMargin)
            actionableFlags.push('Material negative gross profit');
        if (recurringDaySupplyIssue)
            actionableFlags.push('Recurring atypical day-supply pattern');
        if (recurringBrandCash)
            actionableFlags.push('Recurring brand cash claims');
        if (concentrationRisk)
            actionableFlags.push('High payer concentration');
    ...
    const flaggedClaims = pioneerClaims.map((claim) => {
        const findings = [];
        if (hasAtypicalDaySupply(claim, stockSizeMap, inventoryReferenceMap))
            findings.push('Atypical quantity/day-supply ratio');
        if (claim.payerType === 'Cash' && /brand|b$/i.test(String(claim.brandGeneric || '')))
            findings.push('Brand cash claim');
        if (claim.inventoryGroup === '340B' && isMedicaidClaim(claim))
            findings.push('Medicaid should not dispense from 340B');
    ...
    const inventoryManagementRaw = inventoryByKey.map((rows) => {
        ...
        let status = 'Healthy';
        if (b340NeedsReplenishment)
            status = 'Replenish 340B';
        else if (rxNeedsReorder)
            status = 'Reorder RX';
        else if (!expensiveBrand && daysOnHand > 30 && sum(rows.map((row) => Math.max(row.onHand, 0) * Number(row.lastCostPaid || 0))) >= 250)
            status = 'Overstock';
        else if (!expensiveBrand && avgDaily >= 1 && daysOnHand < optimalDays * 0.5)
            status = 'Understock';
        ...
        if (lastMovement) {
            ...
            if (daysSince >= 28 && totalOnHand >= Number(first.stockSize || 0))
                status = 'Return candidate';
        }
    ...
    const familyClaimMix = Object.entries(groupBy(pioneerClaims, (claim) => claimFamilyKey(claim, priceMaps))).reduce((acc, [key, claims]) => {
    ...
    const pairedFinancials = pioneerClaims.map((claim) => grossProfitTuple(claim, priceMaps)).filter(Boolean);
    const financeByPharmacy = PHARMACIES.map((pharmacy) => {
        const claims = pioneerClaims.filter((claim) => claim.pharmacyCode === pharmacy.code);
        const pairs = claims.map((claim) => grossProfitTuple(claim, priceMaps)).filter(Boolean);
